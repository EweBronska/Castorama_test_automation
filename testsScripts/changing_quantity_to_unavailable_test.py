...
class ChangingQuantityToUnavailableTest(BaseTest):
    def test_changing_quantity_to_unavailable(self):
        """
        TC 006 : User changes quantity of the product added to the cart to unavailable in the chosen market
        """
        # Enter home page
        ...
        searched_product_text = search_results_page.get_searched_product_first_title()
        ...
        self.assertEqual(searched_product_text, cart_page.get_product_in_cart_title())
        ...
        # Change product quantity into max available in chosen market plus one
        max_available_quantity = cart_page.get_product_max_available_quantity()
        max_available_quantity_plus_one = str(int(max_available_quantity) + 1)
        cart_page.clear_and_enter_product_new_quantity(max_available_quantity_plus_one)
        product_new_quantity = cart_page.get_product_quantity_value()
        error_message = cart_page.get_product_quantity_unavailable_message()
        time.sleep(5)

        # Expected behaviour:
        # New product quantity is equal to max quantity available in chosen market plus one
        self.assertEqual(max_available_quantity_plus_one, product_new_quantity)
        # Proper message informing about product unavailable quantity is displayed
        self.assertEqual("Wybrana ilość produktu jest aktualnie niedostępna.\nMaksymalna ilość jaką można dodać do koszyka to " + max_available_quantity, error_message)
        # Cart Items Counter is not updated and still equals "1"
        self.assertEqual("1", cart_page.get_cart_items_counter())
        # Cart contents list has still only one position
        self.assertEqual(1, len(cart_page.get_items_in_cart_list()))
        # Product Unit Price is still equal to Total Price, because the product quantity has not changed and is still "1"
        self.assertEqual(cart_page.get_product_unit_price(), cart_page.get_product_total_price())
        # Product Total Price is equal to Cart Summary Price
        self.assertEqual(cart_page.get_product_total_price(), cart_page.get_cart_summary_price())
